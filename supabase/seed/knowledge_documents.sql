-- ─────────────────────────────────────────────────────────────
-- GraveSigns — knowledge corpus seed
-- Run AFTER supabase/schema.sql (which creates knowledge_documents).
-- Idempotent: safe to run repeatedly — it upserts on (slug, version).
--
-- Canonical source of this content is the bundled module
-- src/lib/knowledge/documents/ncgr-code-of-ethics.ts. This seed mirrors it so
-- the DB is the editable source of truth in production while the app keeps a
-- bundled fallback. Editing the row here (or via the dashboard) retunes what the
-- reading engine aligns against — no deploy required.
-- ─────────────────────────────────────────────────────────────

insert into public.knowledge_documents
  (slug, kind, title, source, attribution, version, status, content, metadata)
values (
  'ncgr-code-of-ethics',
  'code_of_ethics',
  'The NCGR Code of Ethics',
  'https://ncgrastrology.org/code-of-ethics/',
  '© NCGR, All Rights Reserved. Revised October 1998.',
  'Revised October 1998',
  'active',
  $content$# The NCGR Code of Ethics
...
*—Revised October 1998*$content$,
  jsonb_build_object(
    'organization', 'National Council for Geocosmic Research (NCGR)',
    'code_label', 'NCGR',
    'operating_summary', $summary$Operating principles distilled from the NCGR Code of Ethics for a death-chart reading offered to the grieving:
...
- Serve the reader's best interest, and where grief needs more than a reading, gently point toward real human support (D.2, D.3).$summary$
  )
)
